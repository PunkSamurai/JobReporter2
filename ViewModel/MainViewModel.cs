...
namespace JobReporter2.ViewModel
{
    public class MainViewModel : ObservableObject
    {
        ...
        public MainViewModel()
        {
            ...
            {
                new ShiftModel {  Name = "First Shift", StartTime = TimeSpan.Zero, EndTime = TimeSpan.FromHours(23).Add(TimeSpan.FromMinutes(59)) }
            };
        ...
        private void AssignShiftsToJobs()
        {
            ...
            {
                var shift = Shifts.FirstOrDefault(s => s.IsInShift(job.StartTime)) ?? Shifts.First();
                job.Shift = shift.ToString(); // Use the existing Shift field
            }

            // Ensure filtered jobs are updated
            FilteredJobs = new ObservableCollection<JobModel>(FilteredJobs);
        }

        public void UpdateJobShifts()
        {
            foreach (var job in AllJobs)
            {
                // Find the shift matching the job's start time
                var shift = Shifts.FirstOrDefault(s =>
                    s.IsEnabled &&
                    ((s.StartTime <= s.EndTime && job.StartTime.TimeOfDay >= s.StartTime && job.StartTime.TimeOfDay < s.EndTime) ||
                     (s.StartTime > s.EndTime && (job.StartTime.TimeOfDay >= s.StartTime || job.StartTime.TimeOfDay < s.EndTime))));

                job.Shift = shift?.Name ?? "Unassigned";
            }

            // Refresh filtered jobs to reflect changes
            FilteredJobs = new ObservableCollection<JobModel>(FilteredJobs);
        }
